...
class Painter:
    ...
    def __calculate_one_job(self, data_path) -> list[int]:
        with open(data_path, "r") as log_file:
            data = [
                [int(j.rstrip()) for j in i.split(",")] for i in log_file.readlines()
            ]
    ...
    def __calculate_sum_all_jobs(self, logs_path_array):
        result_array = None
    ...
    def __draw_graph(
        ...
        ax1.set_title(
            f"avg={round(avg_data)} | CI 95%=({l_bord_CI}, {r_bord_CI})", fontsize=16
        )
